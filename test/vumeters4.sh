...
return view.extend({
	...
	load: function() {
		// If the uci config can't be loaded for any reason (missing file,
		// rpcd hiccup, etc.) don't let the whole view crash with an
		// uncaught RPCError - fall back to the built-in defaults below
		// instead. clampInt()'s fallback argument and uci.get()'s null
		// safety already handle a "no data" case gracefully.
