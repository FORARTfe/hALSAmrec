...
# Bound an external command to $1 seconds. Needed because rpcd's ubus
# "file exec" has no timeout of its own: this daemon's detect/probe path
# is reachable directly from the browser (LuCI view -> fs.exec ->
# luci-vumeters-levels -> this script's "detect" subcommand), and if a
# USB audio device ever wedges mid-negotiation, an unbounded arecord call
# there hangs that single ubus request forever - and since browser polls
# are sequential (the next one only fires after the previous promise
# settles), the whole VU meter page freezes on "Waiting..." permanently.
# Prefers the timeout applet (present in virtually all OpenWrt busybox
# builds); falls back to a manual background+kill implementation.
# Note: a process stuck in kernel D-state (uninterruptible sleep, e.g. a
# genuinely wedged USB driver) cannot be killed by any signal until the
# kernel itself unblocks it - no userspace wrapper can fix that. This
# still bounds OUR wait time in that case: we send the signal and return
# immediately rather than blocking on wait(), leaving the process to be
# reaped whenever (if ever) the kernel releases it.
run_timeout() {
	secs=$1; shift
	...
	i=0
	while [ "$i" -lt "$secs" ]; do
		kill -0 "$cpid" 2>/dev/null || { wait "$cpid" 2>/dev/null; return $?; }
		sleep 1
		i=$((i+1))
	done
...
find_dev() {
	...
	# run_timeout bounds this in case the ALSA layer stalls enumerating
	# devices (rare, but this runs on the browser-facing path too).
	run_timeout 4 arecord -l 2>/dev/null | awk '
...
# run_timeout bounds the open/query: this is the exact call that sits on
# the browser-facing detect path, so it must never be allowed to hang.
# Outputs "channels:rate:buf_time:buf_size" or nothing on failure.
probe() {
	command -v arecord >/dev/null 2>&1 || return 1
	run_timeout 4 arecord -D "hw:$1,$2" --dump-hw-params 2>&1 | awk '
