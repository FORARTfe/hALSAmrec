#!/bin/sh
#
# install-autorecorder.sh — hALSAmrec v4.7 installer
...
#!/bin/sh
#
# /usr/sbin/recorder — hALSAmrec v4.7
...
#
# autorecorder procd init script — hALSAmrec v4.7
# by FORART (https://forart.it/), 2025-26
# GPL v3 — see <https://www.gnu.org/licenses/>
...
#!/bin/sh
#
# rpcd shell plugin for hALSAmrec autorecorder v4.7
# by FORART (https://forart.it/), 2025-26
# GPL v3 — see <https://www.gnu.org/licenses/>
#
...
# [FIX-T] is_running: PID file (O(1)) with /proc liveness check + cmdline
# cross-check to guard against PID reuse.  Falls back to a full /proc scan
# for the narrow window between service start and the first PID-file write.
#
# Replaces "pgrep -f $RECORDER" which fails in two ways on BusyBox:
#   (a) Some builds omit -f; the flag is silently ignored, matching nothing.
#   (b) procd exec's the shebang interpreter, so /proc/PID/cmdline starts
#       with "/bin/sh", not the script path — pgrep -f never matches.
is_running() {
    local pid
    # Primary path: PID file written by recorder on startup
    pid=$(cat "$PIDFILE" 2>/dev/null)
    if [ -n "$pid" ] && [ -d "/proc/$pid" ]; then
        # Guard against PID reuse: verify the cmdline contains our script path
        tr '\000' '\n' < "/proc/$pid/cmdline" 2>/dev/null \
            | grep -qF "$RECORDER" && return 0
    fi
    # Fallback: full /proc scan (covers startup race before PID file exists)
...
# ── 7. Summary ────────────────────────────────────────────────────────────────
printf "\n${BOLD}${GREEN}============================================${RESET}\n"
printf "${BOLD}${GREEN}  hALSAmrec v4.7 — Installation complete!${RESET}\n"
...
printf "\n"
printf "  ${BOLD}v4.7 LuCI fixes:${RESET}\n"
...
printf "        (fixes 'list read *' not acting as wildcard on OpenWrt 21/22)\n"
printf "\n"
