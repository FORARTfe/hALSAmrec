...
function index()
	...
	entry({"admin", "halsamrec"}, alias("admin", "halsamrec", "devices"), _("Audio Devices"), 60).dependent = true
	entry({"admin", "halsamrec", "devices"}, template("halsamrec/devices"), _("Audio Devices"), 10)
	entry({"admin", "halsamrec", "probe"}, call("action_probe"), nil).leaf = true
	entry({"admin", "halsamrec", "service"}, call("action_service"), nil).leaf = true
end
...
endmodule("luci.controller.ALSA", package.seeall)
